...
async function main() {
  ...
  const options = Options.newOptions().addExecutorLzReceiveOption(10000, 0).toHex();

  const sendParamSepolia = {
    dstEid: EID_AMOY,
  ...
  let nativeFee;
  try {
    const feeResponse = await nativeOFT_Sepolia.quoteSend(sendParamSepolia, false);
  ...
  let txSend;
  try {
    txSend = await nativeOFT_Sepolia.send(sendParamSepolia, { nativeFee, lzTokenFee: 0 }, walletSepolia.address, {
      value: sendParamSepolia.amountLD.add(nativeFee), // only once!
      gasLimit: 1_500_000,
    });
    await txSend.wait();
